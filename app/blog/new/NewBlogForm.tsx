...
const NewBlogForm = (props: Props) => {
  ...
  if (!session && status !== "loading") return (
    <div>Paylaşım yapmak için oturum açmalısınız</div>
  )
  ...
  if (submitted) return (
    <div className='py-2 container flex flex-col mt-12'>
      <div className='flex flex-col flex-1 items-stretch justify-center h-full text-left border p-8'><h1 className='text-4xl'>Gönderiniz yayınlandı</h1>
        <Link href={`/blog/${postID}`} className='text-indigo-500 text-xl mt-4'>Görüntülemek için buraya tıklayın</Link>
      </div>
    </div>
  )
  ...
  return (
    <div className='min-h-[calc(100vh-130px)] py-2 container flex flex-col mt-12'><form className='flex flex-col flex-1 items-stretch justify-center h-full text-left' onSubmit={handleSubmit}>
      ...
      <div className="self-start">
        ...
        <label className='text-slate-600 mb-3'>{thumbnail ? "Change Image" : "Önizleme resmi ekle (isteğe bağlı)"}</label>
        <UploadButton
      ...
      <button type="submit" className='w-fit-content bg-indigo-400 px-4 py-2 sm:px-6 sm:py-4 mt-6 border-2 rounded shadow-[0.25rem_0.25rem_0px_0px_rgba(0,0,0,1)]'>Paylaş</button>
    </form></div>
  )
}
